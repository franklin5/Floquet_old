...
//		cout << "loopD is finished." << endl;
    ...
    complex<double> temp;
    for(int ip = lowerbound; ip < _SMAX/2; ++ip) {
...
void cBdG_Bulk::compute(){
  ...
  MPI_Reduce(&chern_rank, &total_chern, 1, MPI_DOUBLE, MPI_SUM, _root, MPI_COMM_WORLD);
  if (_root==_rank) {
